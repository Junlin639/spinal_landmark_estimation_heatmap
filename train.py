...
def main():
    ...
    trainset = SpinalDataset_Heatmap(
        csv_file = args.datapath + '/labels/training/filenames.csv', transform=transform_train,
        img_dir = args.datapath + '/data/training/', landmark_dir = args.datapath + '/labels/training/')
    ...
    criterion = {'L1':nn.L1Loss().cuda(),'MSE':nn.MSELoss().cuda()}
...
def train(trainloader, model, criterion, optimizer, epoch, use_cuda):
    ...
    for batch_idx, batch_data in enumerate(t):
        ...
        losses.update(loss2.data, inputs.size(0))
        ...
        optimizer.step()
    # plot progress
    print('\n Loss: {loss:.4f}'.format(loss=losses.avg,))
    return (losses.avg,0)
